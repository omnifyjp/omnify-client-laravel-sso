...
```tsx
// Runtime utilities from @famgia/omnify-react
import {
  zodRule,
  setZodLocale,
  JapaneseNameField,
...
import { zodRule, setZodLocale, JapaneseNameField } from '@famgia/omnify-react';
import {
  type Customer,
  type CustomerCreate,
  customerSchemas,
...
### 1. Helper Functions (MUST define in every form)
...
For Japanese compound types, use Japanese field components:
...
- [ ] Import from `@famgia/omnify-react`
...
- [ ] Use Japanese field components (JapaneseNameField, etc.)
- [ ] Provide options for Select fields (prefectureOptions, accountTypeOptions)
