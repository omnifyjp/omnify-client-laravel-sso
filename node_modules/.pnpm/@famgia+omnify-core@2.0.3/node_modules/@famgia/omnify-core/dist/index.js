// src/index.ts
import { ok, err } from "@famgia/omnify-types";
...
var Omnify = class {
  ...
  constructor(options) {
    this.options = {
      ...options,
      schemaDir: resolve2(options.schemaDir)
    };
...
import { readdir as readdir2, readFile as readFile2, writeFile as writeFile2, mkdir, rm, access as access2 } from "fs/promises";
...
var VersionStore = class {
  ...
  maxVersions;
  constructor(config) {
    this.omnifyDir = join2(config.baseDir, OMNIFY_DIR);
    this.versionsDir = join2(this.omnifyDir, VERSIONS_DIR);
  ...
  getVersionPath(version, migration) {
    ...
    return join2(this.versionsDir, filename);
  }
  /**
  ...
  async listVersions() {
    ...
    const files = await readdir2(this.versionsDir);
    const versions = [];
    for (const file of files) {
      const parsed = parseVersionFilename(file);
      if (!parsed) continue;
      try {
        const filePath = join2(this.versionsDir, file);
        const content = await readFile2(filePath, "utf-8");
        const versionFile = yaml2.load(content);
        const summary = {
  ...
  async createVersion(snapshot, changes, options) {
    ...
    const content = yaml2.dump(versionFile, {
      lineWidth: -1,
  ...
  async updateCurrentLink(versionFile) {
    const currentPath = join2(this.omnifyDir, CURRENT_FILE);
    const content = yaml2.dump(versionFile, {
      lineWidth: -1,
      noRefs: true,
      quotingType: '"'
    });
    await writeFile2(currentPath, content, "utf-8");
  }
  /**
    ...
    for (const v of toDelete) {
      ...
      for (const file of files) {
        ...
        if (parsed?.version === v.version) {
          await rm(join2(this.versionsDir, file));
          break;
        }
      }
    }
...
import { resolve as resolve3, dirname as dirname2, join as join3, basename as basename2 } from "path";
...
function getKnowledgeDir() {
  const possiblePaths = [
    // Dev: src内で実行している場合
    resolve3(__dirname, "knowledge"),
    // Dist: dist/から実行、src/ai-guides/knowledge
    resolve3(__dirname, "../src/ai-guides/knowledge"),
...
function getConfigDir() {
  const possiblePaths = [
    // Dev: src内で実行している場合
    resolve3(__dirname, "config"),
    // Dist: dist/から実行、src/ai-guides/config
    resolve3(__dirname, "../src/ai-guides/config"),
    // Installed package: node_modules内
    resolve3(__dirname, "../config")
  ];
  for (const path2 of possiblePaths) {
...
function loadRulesConfig() {
  ...
  const rulesPath = join3(configDir, "rules.yaml");
...
function loadKnowledgeFile(source) {
  ...
  const filePath = join3(knowledgeDir, source);
  ...
  for (const rule of config.guides) {
    try {
      ...
      for (const adapter of adapters) {
        ...
        const fullPath = resolve3(rootDir, adapter.outputDir, outputPath);
  ...
  if (cursorAdapter2 && config.cursorRules) {
    for (const rule of config.cursorRules) {
      try {
        ...
        const filename = basename2(rule.source).replace(".mdc.stub", ".mdc").replace(".md.stub", ".md");
        const fullPath = resolve3(rootDir, cursorAdapter2.outputDir, filename);
        writeOutputFile(fullPath, transformed, dryRun);
        result.files.push(fullPath);
  ...
  if (claudeAdapter2 && config.claudeRules) {
    for (const rule of config.claudeRules) {
      try {
        ...
        const filename = basename2(rule.source).replace(".mdc.stub", ".mdc").replace(".md.stub", ".md").replace(".mdc", ".md");
        const fullPath = resolve3(rootDir, ".claude/rules/omnify", filename);
        writeOutputFile(fullPath, transformed, dryRun);
        result.files.push(fullPath);
  ...
  if (antigravityAdapterInstance && config.antigravityRules) {
    for (const rule of config.antigravityRules) {
      try {
        ...
        const filename = basename2(rule.source).replace(".mdc.stub", ".mdc").replace(".md.stub", ".md").replace(".mdc", ".md");
        const fullPath = resolve3(rootDir, antigravityAdapterInstance.outputDir, filename);
        writeOutputFile(fullPath, transformed, dryRun);
        result.files.push(fullPath);
...
function transformForAntigravity(content, rule, placeholders) {
  ...
  if (transformed.startsWith("---")) {
    ...
    if (endIndex !== -1) {
      ...
      const id = basename2(rule.source).replace(".mdc.stub", "").replace(".md.stub", "").replace(".mdc", "").replace(".md", "");
...
function shouldGenerateAIGuides(rootDir) {
  const claudeDir = resolve3(rootDir, ".claude/omnify/guides");
  const cursorDir = resolve3(rootDir, ".cursor/rules/omnify");
